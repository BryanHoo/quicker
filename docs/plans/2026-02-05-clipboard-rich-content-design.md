...
- 隐私最佳实践：尊重剪贴板内容的“瞬态/敏感/自动生成”标记类型（见 5.0），避免将不应被记录的内容写入历史文件。
...
- 目录获取与创建：使用 `FileManager.url(for: .applicationSupportDirectory, in: .userDomainMask, appropriateFor: nil, create: true)` 并 `createDirectory(..., withIntermediateDirectories: true)`；写入文件用 `Data.write(..., options: .atomic)`，避免半写入文件。
- API（建议）：
...
剪贴板历史应用通常会忽略带有以下标记类型的内容，避免记录“瞬态/敏感/自动生成”的剪贴板内容：

- `org.nspasteboard.TransientType`
- `org.nspasteboard.ConcealedType`
- `org.nspasteboard.AutoGeneratedType`
- 以及常见的专有标记类型（例如 `com.agilebits.onepassword`、`de.petermaurer.TransientPasteboardType` 等）

本项目建议：当检测到任一上述类型存在于当前 `NSPasteboardItem.types` 中时，直接跳过本次写入历史（不插入 `ClipboardEntry`）。
...
- `rtfData` 直接持久化
- 用 `NSAttributedString(data:options:documentAttributes:)`（`options[.documentType] = .rtf`）提取 `string` 作为 `text` 预览与 `.string` 降级（解析失败则回退 `.string`）
- 说明：RTF/RTFD 是 Apple 推荐的富文本持久化格式；本期仅做 RTF（不含附件），不支持 RTFD 附件保真。
...
- macOS Pasteboard 隐私收紧：有公开信息称 Apple 计划在后续 macOS 版本中对“后台读取通用剪贴板”弹出提示，并提供 `detect`/`accessBehavior` 等 API 用于仅探测类型而不读取内容。若该变化落地，当前基于轮询读取内容的 `ClipboardMonitor` 可能需要适配（例如先探测类型、降低读取频率、或仅在用户交互后读取）。
